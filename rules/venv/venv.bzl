"""
Python venv rules to work alongside rules_python and mypy rules
"""
# Copyright 2021 cedar.ai. All rights reserved.
#
# Licensed under the Apache License, Version 2.0 (the "License");
# you may not use this file except in compliance with the License.
# You may obtain a copy of the License at
#
#    http://www.apache.org/licenses/LICENSE-2.0
#
# Unless required by applicable law or agreed to in writing, software
# distributed under the License is distributed on an "AS IS" BASIS,
# WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
# See the License for the specific language governing permissions and
# limitations under the License.

PyVenvInfo = provider(
    "TODO: docs",
    fields = {
        "python": "Path to python interpreter",
    },
)

PYTHON_TOOLCHAIN_TYPE = "@bazel_tools//tools/python:toolchain_type"

def _py_venv_impl(ctx):
    toolchain_depset = ctx.toolchains[PYTHON_TOOLCHAIN_TYPE].py3_runtime.files or depset()
    toolchain_files = {f: None for f in toolchain_depset.to_list()}

    imports = []
    for dep in ctx.attr.deps:
        if PyInfo not in dep:
            continue
        imports.extend([i for i in dep[PyInfo].imports.to_list() if i not in imports])

    deps = depset(transitive = [dep[DefaultInfo].default_runfiles.files for dep in ctx.attr.deps])
    data = depset(transitive = [data[DefaultInfo].files for data in ctx.attr.data])
    out = ctx.actions.declare_file("_virtualenv_deps.json")

    files = []
    for dep in deps.to_list() + data.to_list():
        # Skip files that are provided by the python toolchain.
        # They don't need to be in the venv.
        if dep in toolchain_files:
            continue

        typ = "S" if dep.is_source else "G"
        files.append({"p": dep.short_path, "t": typ})

    doc = {
        "always_link": ctx.attr.always_link,
        "commands": ctx.attr.commands,
        "files": files,
        "imports": imports,
        "workspace": ctx.workspace_name,
    }
    ctx.actions.write(out, json.encode(doc))

    # Initialize virtualenv. Symlinking occurs on run.
    virtualenv_directory = ctx.actions.declare_directory("virtualenv")
    ctx.actions.run(
        outputs = [virtualenv_directory],
        inputs = [out],
        executable = ctx.executable._build_env,
        env = {
            "BUILD_ENV_INPUT": out.path,
            "VENV_LOCATION": virtualenv_directory.path,
        },
    )

    dummy_executable = ctx.actions.declare_file("_dummy_exec")
    ctx.actions.write(
        output = dummy_executable,
        is_executable = True,
        content = "echo %s" % ctx.label.name,
    )

    runfiles = ctx.runfiles(
        files = [out, virtualenv_directory],
    )

    # TODO(alexmirrington): Generate proper executable and pass through in
    # default info to be called by other rules
    # TODO(alexmirrington): Work out why mypy is mysteriously working now that we create folders but don't symlink?
    return [
        DefaultInfo(
            files = depset(direct = [out, virtualenv_directory]),
            executable = dummy_executable,
            runfiles = runfiles,
        ),
        PyVenvInfo(
            python = "%s/bin/python" % virtualenv_directory.path,
        ),
    ]

py_venv = rule(
    implementation = _py_venv_impl,
    attrs = {
        "always_link": attr.bool(),
        "commands": attr.string_list(),
        "data": attr.label_list(),
        "deps": attr.label_list(),
        "output": attr.output(),
        "_build_env": attr.label(
            default = Label("//rules/venv:build_env"),
            executable = True,
            cfg = "exec",
        ),
    },
    toolchains = [PYTHON_TOOLCHAIN_TYPE],
    executable = True,
)

# def py_venv(name, deps = None, data = None, extra_pip_commands = None, always_link = False, venv_location = None, **kwargs):
#     deps = deps or []
#     data = data or []
#     extra_pip_commands = extra_pip_commands or []

#     deps_name = "_" + name + "_deps"
#     out_name = deps_name + ".json"
#     out_label = ":" + out_name
#     _py_venv_deps(
#         name = deps_name,
#         deps = deps,
#         data = data,
#         commands = extra_pip_commands,
#         always_link = always_link,
#         output = out_name,
#         **kwargs
#     )

#     env = {
#         "BUILD_ENV_INPUT": "$(location " + out_label + ")",
#         "LINK_RUNFILES": "true"
#     }

#     if venv_location:
#         env.update({"VENV_LOCATION": venv_location})
#     # TODO(alexmirrington): Avoid macro?
#     py_binary(
#         name = name,
#         srcs = ["//rules/venv:build_env.py"],
#         deps = ["@rules_pyvenv//vendor/importlib_metadata"],
#         data = [out_label] + deps + data,
#         main = "//rules/venv:build_env.py",
#         env = env,
#         **kwargs
#     )
